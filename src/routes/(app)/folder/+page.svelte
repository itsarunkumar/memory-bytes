<script lang="ts">
	...
	import { Files, FolderInput, FolderOpen, Library, Trash } from 'lucide-svelte';
...
<Modal isOpen={showModal} on:close={toggleModal} class="w-full h-full">
	...
	<p class="text-muted-foreground text-sm">create a folder where you can organize your bytes</p>
	<form
		action="?/createFolder"
		method="post"
		use:enhance={() => {
	...
	>
		...
		<div class="flex gap-5 justify-end items-center">
			...
			<button type="submit" class="px-4 py-2 text-green-500">Create</button>
		</div>
	</form>
</Modal>
		...
		>
			...
			<div class="flex gap-2 justify-between w-full items-center border-t">
				...
				>
					...
					<button class="flex gap-2 items-center">Delete <Trash class="w-4 h-4" /> </button>
				</form>
			</div>
		</div>
	{/each}
